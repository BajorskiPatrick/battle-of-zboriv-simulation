            ...
            for tileset in map_data.tilesets:
                ...
                tileset_info = {
                    ...
                    "tilewidth": getattr(
                        tileset,
                        "tilewidth",
                        getattr(self.model.map_data, "tilewidth", 16),
                    ),
                    "tileheight": getattr(
                        tileset,
                        "tileheight",
                        getattr(self.model.map_data, "tileheight", 16),
                    ),
                ...
                if tileset_rel_path:
                    map_dir = os.path.dirname(
                        os.path.abspath(self.model.map_data.filename)
                    )
                    tileset_path = os.path.normpath(
                        os.path.join(map_dir, tileset_rel_path)
                    )
                    tileset_info["image_path"] = tileset_path
                    if os.path.exists(tileset_path):
                        try:
                            tileset_info["image"] = Image.open(tileset_path).convert(
                                "RGBA"
                            )
                            print(f"✓ Załadowano tileset image: {tileset_path}")
                        except Exception as e:
                            print(
                                f"⚠️ Nie można załadować obrazu tilesetu {tileset_path}: {e}"
                            )
                    else:
                        print(f"⚠️ Plik tilesetu nie istnieje: {tileset_path}")
                else:
                    print(
                        f"⚠️ Nie znaleziono ścieżki do obrazu tilesetu dla firstgid={tileset_info['firstgid']}"
                    )
    ...
    def _get_tile_image(self, gid):
        ...
        try:
            ...
            if not columns or columns == 0:
                columns = max(
                    1, (img_w - 2 * margin + spacing) // (ts_tile_w + spacing)
                )
        ...
        for y in range(height):
            for x in range(width):
                ...
                if c_val > 0 or k_val > 0:
                    ...
                    k_intensity = min(
                        1.0, math.log(k_val + 1) / math.log(max_val + 1) * 2.5
                    )
        ...
        if morale_percent > 0:
            ...
            draw.rectangle(
                [
                    x - bar_width // 2,
                    morale_y - bar_height // 2,
                    x - bar_width // 2 + morale_fill_width,
                    morale_y + bar_height // 2,
                ],
                fill=(0, 200, 200),
            )
